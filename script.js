(function() {
    // function to store items to the local storage
    ...
    function addCartItems(value) {
        ...
        // get the empty space for the cart 
        ...
        // add all the content to the a table
    ...
    var itemTotals = parseInt(sum) + (sum * 0.14);
    localStorage.setItem("itemsTotal", itemTotals)

    // Cart status

    if (cartArray.length > 0) {
        var cartContent = document.querySelector('.cart-contents').innerHTML = '<strong>Total:</strong> $' + itemTotals.toFixed(2);
    }
